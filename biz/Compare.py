    ...
    def compTablesStructure(self, resTable, tagTable, sequential="1", col_properties="1"):
        if isinstance(resTable, Table) and isinstance(tagTable, Table):
                ...
                if sequential == "1":
                    for i in range(len(tagColumns)):
                        if tagColumns[i] != resColumns[i]:
                            list_msg.append("target:%s sequential != refence:%s sequential!" % (
                            tagTable.getName(), resTable.getName()))
            ...
            if col_properties == "1":
                ...
                dict_res = {structure[0]: structure for structure in resTable.getStructure() if
                            structure[0] in columns_both}
                dict_tag = {structure[0]: structure for structure in tagTable.getStructure() if
                            structure[0] in columns_both}
                        ...
                        if len(resTable.getPrimarykey()) > 0:  # 主键不为空，要进行详细对比
                            ...
                            if not bool_row_status:
                                ...
                                list_msg.append("%s line %d is not found in %s\n(%s)" % (
                                    resTable.getInfo(), line + 1, tagTable.getInfo(),
                                    ",".join(map(lambda col: str(col), resRow))
                                ))
            if len(list_msg) == 1:
